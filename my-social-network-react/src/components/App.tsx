...
function App() {
  ...
  const handleCardLike = (card: Card) => {
    ...
    const newLikes = isLiked ? card.likes.filter((like) => like.id !== user.id) : [...card.likes, { id: user.id }];

    api
      .setCardLikes(card.id, newLikes)
      .then((updatedCard) => {
        setInitialCards((currentCards) =>
          currentCards.map((currentCard) => (currentCard.id === card.id ? updatedCard : currentCard)),
        );
      })
  ...
  const handleUpdateUser = (userId: number, userName: string, userDescription: string) => {
    api
      .updateUser(userId, userName, userDescription)
      .then((updatedUser) => {
        setUser(updatedUser);
      })
  ...
  const handleUpdateAvatarUser = (userId: number, userAvatar: string) => {
    api
      .updateAvatarUser(userId, userAvatar)
      .then((updatedUser) => {
        setUser(updatedUser);
      })
    ...
    <BrowserRouter>
      <CurrentUserContext.Provider value={user}>
        ...
        <QestionModal
          ...
          card={cardToDelete}
        />
        <ImageModal card={isSelectedCard} isImageModalOpen={isImageModalOpen} setImageModalOpen={setImageModalOpen} />
